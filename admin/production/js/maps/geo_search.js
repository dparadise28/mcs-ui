...
L.Control.GeoSearch = L.Control.extend({
    ...
    _config: {
        country: '',
        searchLabel: 'search for address/store ...',
        notFoundMessage: 'Sorry, that address or name you entered could not be found.',
    ...
    GetLocations: function(qry, callback) {
        ...
        var results = geocoder.geocode(parameters, function(data){
            ...
            if(typeof callback == 'function')
                callback(results);
            map_results = results;
            store.data_models.store.address = map_results;
        });
    },
});
...
// Create the map
var map = L.map('store_locator_map').setView([40.7128, -74.0059], 9);
